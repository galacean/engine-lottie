...
async function init() {
  ...
  const demos = {
    ...
    碎片: ["https://mdn.alipayobjects.com/huamei_w1o8la/afts/file/A*GxWPRbeur0MAAAAAAAAAAAAADsB_AQ"],
    "福袋(金额文字)": [
  ...
  const lottieResource = await engine.resourceManager.load<LottieResource>({
    url: "https://mdn.alipayobjects.com/oasis_be/afts/file/A*vGadR5BXK6kAAAAAAAAAAAAADkp5AQ/CCB.json",
    type: "EditorLottie"
  });
	const lottieResource1 = await engine.resourceManager.load<LottieResource>({
    url: "https://mdn.alipayobjects.com/oasis_be/afts/file/A*hBHUQJOpmxUAAAAAAAAAAAAADkp5AQ/CMB.json",
    type: "EditorLottie"
  });
	const lottieResource2 = await engine.resourceManager.load<LottieResource>({
    url: "https://mdn.alipayobjects.com/oasis_be/afts/file/A*9RpGQYQ7FIUAAAAAAAAAAAAADkp5AQ/SPABANK.json",
    type: "EditorLottie"
  });
	const lottieEntity = root.createChild('lottie');
	const lottieComponent = lottieEntity.addComponent(LottieAnimation);
	lottieComponent.resource = lottieResource;
	lottieComponent.isLooping = true;
	lottieComponent.speed = 1;
	lottieComponent.play();

	setTimeout(() => {
		lottieComponent.resource = lottieResource1;
		setTimeout(async () => {
			lottieComponent.resource =  await engine.resourceManager.load<LottieResource>({
				url: "https://mdn.alipayobjects.com/oasis_be/afts/file/A*vGadR5BXK6kAAAAAAAAAAAAADkp5AQ/CCB.json",
				type: "EditorLottie"
			});;
		}, 1000);
